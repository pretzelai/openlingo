...
export function ChatLayout({ conversations, children }: ChatLayoutProps) {
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-3">
        ...
        {groups.map((group) => (
          <div key={group.label} className="mb-3">
            <p className="px-2 py-1.5 text-xs font-semibold text-lingo-text-light">
              {group.label}
            </p>
              ...
              return (
                ...
                >
                  <Link
                    href={`/chat/${conv.id}`}
                    onClick={() => setSidebarOpen(false)}
                    className="min-w-0 flex-1 px-2 py-2 text-sm font-medium truncate"
                  >
                    {conv.title}
                  </Link>
                  <button
  ...
  return (
    <div className="relative flex h-[calc(100vh-8rem)] md:h-[calc(100vh-6rem)]">
      {/* Mobile overlay */}
      {sidebarOpen && (
        <div
